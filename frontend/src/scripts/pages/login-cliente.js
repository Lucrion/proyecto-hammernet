...
async function handleLogin(e){ e.preventDefault(); const usernameInput=(document.getElementById('username')?.value||'').trim(); const baseRut = usernameInput.includes('-') ? usernameInput.split('-')[0] : usernameInput; const username = digitsOnly(baseRut); const password=document.getElementById('password')?.value||''; if(!username||!password){ showStatus('Por favor, ingrese RUT y contraseña','error'); return; }
  ...
  try{ const controller=new AbortController(); const timeoutId=setTimeout(()=>controller.abort(),10000);
    const formData=new URLSearchParams(); formData.append('username', username); formData.append('password', password); formData.append('grant_type','password');
    const opts={ method:'POST', body:formData, signal:controller.signal, ...corsConfig };
    const response=await fetch(`${API_URL}/auth/login-cliente`, opts); clearTimeout(timeoutId);
    if(!response.ok){ const txt=await response.text(); let msg = response.status===403 ? 'Acceso restringido a clientes' : (response.status===401 ? 'Credenciales incorrectas' : `Error de autenticación (${response.status})`); try{ const j=JSON.parse(txt); if(j?.detail) msg = typeof j.detail==='string'? j.detail : JSON.stringify(j.detail); }catch{} showStatus(msg,'error'); showLoading(false); return; }
    const data=await response.json(); const user={ id_usuario:data.id_usuario, nombre:data.nombre, rut:data.rut ?? username, rol:data.role||'cliente' };
    ...
    showStatus('Autenticación exitosa. Redirigiendo...','success'); setTimeout(()=>{ window.location.href='/'; },800);
  } catch(err){ showLoading(false); showStatus('Error de autenticación','error'); }
}

document.addEventListener('DOMContentLoaded', async ()=>{ const form=document.getElementById('loginForm'); const label=document.getElementById('usernameLabel'); const input=document.getElementById('username'); const create=document.getElementById('createAccountContainer'); if(label) label.textContent='RUT'; if(input) input.placeholder='20.347.793-7'; if(create) create.classList.remove('hidden'); if(form) form.addEventListener('submit', handleLogin); });
